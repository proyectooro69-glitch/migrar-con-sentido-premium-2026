...
function WhatsAppButton({ children = 'Escribir por WhatsApp', className = '' }: { children?: React.ReactNode; className?: string }) {
  return <a href={whatsappUrl} target="_blank" rel="noreferrer" className={`inline-flex min-h-12 items-center justify-center gap-2 bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${className}`}><MessageCircle className="h-4 w-4" />{children}</a>
}
  ...
  return <div className="min-w-0 overflow-hidden bg-background">
    ...
    <main>
      <section id="inicio" className="relative flex min-h-[720px] items-end overflow-hidden bg-primary pb-16 pt-28 lg:min-h-[820px] lg:items-center lg:pb-0">
        <div className="absolute inset-0 cinematic-hero" aria-label="Ruta visual de Cuba a España"><img src={heroImage} alt="Ruta de viaje entre Cuba y España con avión y puntos de llegada" className="h-full w-full object-cover object-[62%_center] lg:object-center" /><div className="hero-vignette" /></div>
